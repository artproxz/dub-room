...
const dataDir = await mkdtemp(path.join(os.tmpdir(), 'dubroom-daw-smoke-'));
...
try {
  ...
  result = await json(`${base}/api/rooms/${roomId}/recording/start`, { method:'POST', body:JSON.stringify({ participantId:'u1' }) });
  assert.equal(result.res.status,200); assert.deepEqual(result.body.armedParticipantIds.sort(), ['u1','u3']);
  ...
  res = await fetch(`${base}/api/rooms/${roomId}/sessions/${sessionId}/clips/u1`, {
  ...
  console.log('✓ DubRoom DAW smoke test passed');
} finally {
